...
bool Automaton::CheckDFA() {
  // Condiciones para ser DFA:
  // 1. no epsilon-transiciones
  // 2. Para cada estado y cada símbolo del alfabeto, como mucho una transición
  for (const auto& state_pair : states_) {
    const State& state = state_pair.second;
    std::multimap<symbol, int> transitions = state.getTransitions();
    if (transitions.find(EMPTY_STRING) != transitions.end()) {
      return false;
    }
    for (symbol c : alphabet_.getSet()) {
      auto range = transitions.equal_range(c);
...
void Automaton::ExportRegularGrammar() {
  std::ofstream output_file{"eq_regular_grammar"};
  if(CheckDFA()) {
    for(symbol c : alphabet_.getSet()) {
      output_file << c << " ";
    }
    ...
    output_file << "The loaded FA is not a DFA, so the Regular Grammar can't be obtained.";
  }
}